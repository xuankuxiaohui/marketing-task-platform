...
- 禁止新增 `any`
  - `catch (e: unknown)` 后使用类型守卫
  - `defineModel<T>()` 使用具体泛型类型
...
以下字符串多处使用时应提取到 `constants/index.ts`：
...
以下逻辑多处重复时适合提取为 composable：

| 候选 Composable | 适用场景 |
|---|---|
| `useStatusLabel()` | 状态标签映射被多个页面复用 |
| `usePeriodLabel()` | 周期标签映射被多个页面复用 |
| `useRequest()` | 多个组件重复维护 loading/error/data |
| `useUserContext()` | 用户上下文读写需要跨组件复用 |
...
| TypeScript `any` | 逐步替换为 `unknown` 或具体类型 |
| 响应拦截器 | 添加统一错误处理 |
