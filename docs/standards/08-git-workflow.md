...
> 流程取 [GitHub flow](https://docs.github.com/en/get-started/using-github/github-flow)（短命分支 → PR → `main`），提交说明取 [Conventional Commits](https://www.conventionalcommits.org/)。

## 1. 长期分支

| 分支 | 角色 |
|------|------|
| `main` | 唯一长期分支。随时可构建。禁止直接 `push`（有远端保护后） |
...
1. **MUST** 一个分支只对应一件事（通常一个任务号）。编组 J（任务 44–49）**MUST NOT** 在任务 43 验收前从 `main` 开功能分支合入。
...
- 本地整理 **可以** rebase 自己的短命分支；**MUST NOT** `push --force` 到 `main`
...
4. 合入方式：**squash merge**，保持 `main` 线性。
...
远端（GitHub）对 `main` 开启：禁止直推、PR 必审、状态检查通过才能合。
