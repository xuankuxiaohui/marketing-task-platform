# PROJECT_STATUS
> 阶段：**编码（编组 E 任务 21 评审修复已 push、未合）** / 当前任务：**21 修 PR #4 issue #8，未合 master** / 更新：2026-08-19
...
- 进行中：PR #4 评审缺陷 #5/#6/#7/#8 已修并 push；禁止 merge
...
- 登录拒绝是提交结果：`@Transactional login` 返回 `AuthAttempt`，Controller `orThrow()`。错误密码 / 验证码错 / 锁定拒绝提交 `failed_attempts`、锁定与失败 `audit.log`。未用 REQUIRES_NEW

## 改过的核心文件

- `server/domain-identity/`：`AdminAuthService` / `PortalAuthService` 登录返回 `AuthAttempt`；`AdminAuthServiceTransactionalTest`、`PortalAuthServiceTransactionalTest`、`LoginLockCommitIT`
...
- 矩阵覆盖：`LoginLockPropertyTest`；`AdminAuthServiceTransactionalTest`（Spring 代理：5 次错密提交锁定 + 5 条 FAILURE）；`LoginLockCommitIT`、`LoginAuditIT`、`PasswordChangeSessionIT`、`NamespaceIsolationIT` 交叉令牌 401 未削弱
- 未跑项及原因：本机无 Docker，`*IT` 留给 CI（未削弱断言，未用 H2 / Embedded Redis）

## 已知问题（只写已证实）

- 任务 21 PR #4 评审 #5/#6/#7/#8 已修，禁止 merge 直至评审与 CI 通过
