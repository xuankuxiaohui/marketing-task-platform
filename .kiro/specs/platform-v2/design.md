...
> 版本：**v2.13**　需求：[requirements.md](requirements.md) v3.9　选型：[component-selection.md](component-selection.md)
...
>
> v2.13 相对 v2.12：**按章拆册**。§ 编号不变；引用仍写 `design §x.y`；跳转搜 `<!-- §x.y -->`（不行号）。`ad:position` 明确为 P0 预留名 / 任务 48 接线。
