<template>
  <el-card>
    <template #header>
      <div class="header">
        <div>
          <span class="page-title">客户端用户管理</span>
          <p class="page-sub">管理C端用户账号</p>
        </div>
      </div>
    </template>
    ...
    <el-form :inline="true" class="filter-bar">
      <el-form-item label="关键字">
        <el-input v-model="filters.keyword" placeholder="用户名/昵称/省份/角色" clearable style="width:240px" @keyup.enter="search" />
      </el-form-item>
      <el-form-item>
...
defineOptions({ name: 'ClientUserList' })
import { onMounted, reactive, ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { listClientUsers, resetClientUserPassword, toggleClientUserEnabled, kickClientUser } from '../../api/client-user'
...
async function load() {
  ...
  try {
    ...
    const { data } = await listClientUsers(params)
...
async function handleResetPassword(id: number) {
  try {
    const { data } = await resetClientUserPassword(id)
...
async function handleToggleEnabled(id: number) {
  try {
    await toggleClientUserEnabled(id)
